...
void *process_thread(void *data)
{
  ...
  while (1)
  {
    ...
    while (1)
    {
      if (curr != NULL && pthread_mutex_trylock(&curr->mutex) == 0)
      {
        if (curr->next != NULL && pthread_mutex_trylock(&curr->next->mutex) == 0)
        {
...
void *swap_thread(void *data)
{
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      if (curr1 != NULL && pthread_mutex_trylock(&curr1->mutex) == 0)
      {
        if (curr1->next != NULL && pthread_mutex_trylock(&curr1->next->mutex) == 0)
        {
          if (curr1->next->next != NULL && pthread_mutex_trylock(&curr1->next->next->mutex) == 0)
          {
            curr2 = curr1->next;
            ...
            curr2 = curr1->next;
            pthread_mutex_unlock(&tmp->mutex);
            pthread_mutex_unlock(&curr1->mutex);
            pthread_mutex_unlock(&curr2->mutex);
          }
          else
          {
            tmp = curr1;
