...
void *process_thread(void *data)
{
  ...
  while (1)
  {
    ...
    while (1)
    {
      if (curr != NULL && pthread_rwlock_tryrdlock(&curr->rwlock) == 0)
      {
        if (curr->next != NULL && pthread_rwlock_tryrdlock(&curr->next->rwlock) == 0)
        {
...
void *swap_thread(void *data)
{
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      if (curr1 != NULL && pthread_rwlock_trywrlock(&curr1->rwlock) == 0)
      {
        if (curr1->next != NULL && pthread_rwlock_trywrlock(&curr1->next->rwlock) == 0)
        {
          if (curr1->next->next != NULL && pthread_rwlock_trywrlock(&curr1->next->next->rwlock) == 0)
          {
            curr2 = curr1->next;
